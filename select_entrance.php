...
$stmt = $pdo -> prepare('
    SELECT * FROM targetSchool_table ORDER BY entrance ASC
    ');
$status = $stmt->execute();
...
if ($status === false) {
    ...
    while ($result = $stmt->fetch(PDO::FETCH_ASSOC)) {
        ...
        $entranceTime = new DateTime(h($result['entrance']));
        $formattedEntrance = $entranceTime->format('Y-m-d H:i');

        // リンクをクリックするとGET送信される➡detail.php
        $view .= '<a href="detail.php?id='.h($result['id']).'">';
        $view .= h($result['id']) . ' : ' . $formattedTime . ' : ' . h($result['station']). ' : ' . h($result['schoolName']). ' : ' . h($result['subject']). ' : ' . $formattedEntrance;
        $view .= '</a>';
